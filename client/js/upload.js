...
(function () {
    ...
    upload_button_upload.addEventListener('click', function () {
        ...
        instance.post('/upload_single', formData).then(data => {
            if (+data.code === 0) {
                alert(`上传成功~~,通过 ${data.servicePath} 访问`)
                return
            }
            return Promise.reject(data.codeText)
        }).catch(reason => {
            alert(`文件上传失败，${reason.message}`)
        }).finally(() => {
            clearHandle()
            changeDisable(false)   // 切换按钮为可用 
        })
    })
    ...
    upload_inp.addEventListener('change', function () {
        ...
        // 在input表单中通过accept属性放入允许上传的扩展名 ⌈ 方案二 ⌋
